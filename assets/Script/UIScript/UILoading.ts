...
export default class UILoading extends UITips {
    ...
    async showEffect() {
        ...
        await Promise.all([
            CocosHelper.runTweenSync(this.view.Left, cc.tween().to(0.3, {x: -225}, cc.easeIn(3.0))),
            CocosHelper.runTweenSync(this.view.Right, cc.tween().to(0.3, {x: 225}, cc.easeIn(3.0)))
        ]);
    }

    async hideEffect() {
        let len = AdapterMgr.inst.visibleSize.width/2 + this.view.Left.width/2;
        this.view.Left.x = -225;
        this.view.Right.x = 225;
