...
class LinePointsGizmo extends Editor.Gizmo {
    init() {
      // 初始化一些参数
    }
    ...
    onCreateMoveCallbacks() {
      ...
      return {
        ...
        update: (dx, dy, event, i, type) => {
          ...
          if (!start_vertex) {
            start_vertex = target.line[i].clone();
          }
          target.line[i].x = start_vertex.x + dx / this._view.scale;
          target.line[i].y = start_vertex.y + dy / this._view.scale;
    ...
    onUpdate() {
      ...
      // 移动 svg 工具到坐标
      this._tool.plot(target.line, position);
    }
...
module.exports = LinePointsGizmo;
